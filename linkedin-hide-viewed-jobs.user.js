...
(function () {
  ...
  const NORMALIZED_KEYWORDS = VIEWED_KEYWORDS.map(normalizeForMatch).filter(function (keyword) {
  ...
  function isViewedJobCard(card) {
    ...
    if (
      cardContainsViewedInDescendants(card, '[aria-label], [title], span, small, div, p, time', 100)
    ) {
      return true;
    }
  ...
  function hasViewedStateInScope(scope) {
    ...
    return cardContainsViewedInDescendants(
      scope,
